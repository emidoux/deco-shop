/**
 * <RepisasProductsList> es un componente que contiene al listado de productos obtenidos
 * por medio del Fetch a la API y se los filtra por categoria "Repisas".
 */
...
const RepisasProductsList = () =>{
     ...
     * y vuelve a renderizar el componente, listando las repisas en el DOM.
     */
    useEffect(async ()=>{
        const getData = await fetch(API);
        const data = await getData.json();
        const filtered = data.data.filter(item => item.attributes.category === "Repisas") // Filtro del array
        // de productos con categoria "Repisas"
        setProduct(filtered);
